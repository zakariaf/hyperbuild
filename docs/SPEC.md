...
7. **Hard gates.** A stage is complete only when its gate's checklist passes. Gate
   ...
   Max 2 fix rounds — the second only if a Tier-0 signal changed — then the run stays blocked and says so honestly.
...
| 3.5 | hyperbuild-3-5-research-audit | AREA 01's CRITIQUE + INDEX PHASES (runs after 2 AND 3 both complete, and after every verify/ file has landed): hb-corpus-critics read the WHOLE area corpus for contradictions BETWEEN dimensions, cherry-picking, and coverage holes → `research/01-product-and-market/critique/<critic-name>.md` (each separating `[VERIFIED]` from `[OPEN]`); 1 hb-research-critic fills the `live-evidence` seat and runs its enumerated C1–C7 checklist (syndication is not consensus — cluster derivative copies, they argue with the weight of ONE source — plus source independence, verbatim quote integrity at the URL, staleness, live spot-checks outside verify/, sample frame, and demand-vs-supply), reporting all seven rows in `## Checklist coverage`. Orchestrator then applies every CONFIRMED finding AND every verify/ correction to the `author/` docs — REFUTED claims never survive into a synthesis, PARTIALLY_TRUE claims carry their correction, nothing is silently deleted, `research/` files are NOT rewritten — and writes `research/01-product-and-market/_INDEX.md` (every agent by phase, file sizes, verdicts) | 2 critic seats std / 3 premier, one distinct lens each — hb-research-critic owns the `live-evidence` seat in area 01 (enumerated C1–C7, all seven rows mandatory in its `## Checklist coverage` table), hb-corpus-critic fills the rest (`completeness`, plus `domain:<slug>` at premier) |
...
| 16 | hyperbuild-16-ship-gate | THE gate: full test suite green; lint/analyzer clean; ALL generated-skill scripts/*.sh gates pass; every task status=done; every epic's acceptance criteria checked; PRD coverage matrix complete; TRACEABILITY CHAIN walked mechanically — every must/should feature F-NN → its `features/NN-*.md` exists → ≥1 task with `features: [F-NN]` all status done → every file in those tasks' `files:` lists exists in app/ → the tests those tasks added pass — a break ANYWHERE in the chain blocks the ship; git working tree clean with wave/epic commit history present; app builds/runs (platform-appropriate build cmd) → `gates/ship-report.md`. Failures: fix artifacts, re-run gate, max 2 rounds (round 2 only if a Tier-0 signal changed), else blocked + honest report. Final message: what was built, how to run it, test count, known gaps | 1 hb-gate-verifier |
...
| hb-corpus-critic | opus | Read, Grep, Glob, Write | Reads a WHOLE research area under ONE assigned lens — every `research/` dimension file plus every `verify/` verdict — hunting the defect class no single-claim check can see: contradictions BETWEEN dimensions (three dimensions each shipping a different, internally consistent API for one service), cherry-picking, coverage holes, claims whose support collapses when the corpus is read together. Writes `research/<area>/critique/<critic-name>.md`, separating what it actually ran or read (`[VERIFIED]`) from what it merely reasoned about (`[OPEN]`); 2 critic seats per area at standard, 3 at premier, one distinct lens each — in area 01 one seat is `hb-research-critic` under the `live-evidence` lens, so this agent's seats there are `completeness` (+ `domain:<slug>` at premier) and never `skeptic`; NEVER edits another agent's file |
| hb-research-critic | opus | Read, Grep, Glob, WebSearch, WebFetch, Write | The `live-evidence` seat on area 01's critique panel — defined by a capability (`WebSearch`/`WebFetch`, which `hb-corpus-critic` does not have) plus an ENUMERATED checklist run in full every time: C1 syndication clustering + recount + rank re-derivation · C2 source independence · C3 verbatim quote integrity fetched at the URL · C4 staleness vs each competitor's own last release · C5 live spot-checks strictly outside the `verify/` set · C6 sample-frame coverage · C7 demand-vs-supply collision. All seven rows are mandatory in its `## Checklist coverage` table (`none` is a result; a missing row is an incomplete file). Findings to `research/01-product-and-market/critique/research-audit.md` with `lens: live-evidence` frontmatter; NEVER edits the synthesis docs itself |
