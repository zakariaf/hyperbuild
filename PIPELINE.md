# PIPELINE.md — appbuilder architecture

Appbuilder is a two-stage, 16-step Claude Code pipeline (steps 1–12 plus half-steps 3.5
...
The entry skill `appbuilder` is a thin router; each step is its own skill under
`.claude/skills/appbuilder-N-<name>/` (18 step skills + the router + `appbuilder-choose`
= 20 skill directories); the 19 subagents live in `.claude/agents/ab-*.md`.
...
6. **Adversarial by construction.** Drafts, plans, and code are attacked by critics
   (`ab-code-critic`, `ab-spec-critic`, `ab-ux-critic`) that emit findings JSON and NEVER
   edit. Fixes are applied as surgical edits by a tool-locked patcher (`ab-patcher`,
...
## Stage A — PLAN (`/appbuilder <idea>`)

### Step 1 — Intake (`appbuilder-1-intake`)
...
### Step 2 — Market recon (`appbuilder-2-market-recon`)

Competitor discovery, then deep per-competitor analysis: latest versions, feature sets,
changelogs, pricing, positioning, store ratings. Spawns 1 `ab-competitor-scout`, then
one `ab-competitor-analyst` per competitor in ONE message — 6–8 competitors at
...
### Step 3 — Social mining (`appbuilder-3-social-mining`)
...
intensity. Spawns 4 `ab-sentiment-miner` in parallel (one per platform group); 25–40
...
### Step 3.5 — Research audit (`appbuilder-3-5-research-audit`)

The ADVERSARIAL RESEARCH AUDIT — runs only after steps 2 AND 3 are both complete.
Spawns 1 `ab-research-critic` to attack `research/competitor-landscape.md` +
...
### Step 4 — Product spec (`appbuilder-4-product-spec`)
...
`ab-feature-author` in parallel (the roster split into batches). Downstream: steps 6–8 read feature specs for real
...
### Step 5 — Stack research (`appbuilder-5-stack-research`)
...
`ab-stack-researcher` in ONE message (one per topic); 8–12 sources per topic at
...
### Step 6 — Design research (`appbuilder-6-design-research`)
...
`research/harvest/`, then gap-fill). Spawns 3 `ab-design-researcher` in parallel (one
...
### Step 7 — Design systems (`appbuilder-7-design-systems`)
...
`ab-mockup-smith` (screens split per design). Screen count: every mockable PRD screen,
...
### Step 9 — Skill research (`appbuilder-9-skill-research`)
...
licenses). Spawns 1–2 `ab-stack-researcher`. Steps 8 and 9 share no inputs and run as
...
### Step 10 — Skill forge (`appbuilder-10-skill-forge`)
...
Spawns 5 `ab-skill-smith` in ONE message (one per skill). These skills are what
`ab-implementer` and the critics enforce during Stage B — and the script gates are run
...
### Step 11 — Epics (`appbuilder-11-epics`)
...
`ab-epic-planner` (breakdown), then 3–6 `ab-task-author` in parallel (one per epic
batch), then `ab-spec-critic` for the coverage audit. Full contract: `epics/README.md`.

**Artifacts:** `epics/00-overview.md`, `epics/NN-<slug>/epic.md`,
`epics/NN-<slug>/task-NN-<slug>.md`.

### Step 12 — Design gate (`appbuilder-12-design-gate`)

Runs the [design gate checklist](#gate-1--design-gate-step-12) via 1 `ab-gate-verifier`,
...
asks for `/appbuilder-choose a|b|c`. Manifest: `blocked_on: "design-choice"`.

**Artifacts:** `runs/<run_tag>/gates/design-gate-report.md`.

---

## The checkpoint — `/appbuilder-choose <a|b|c>`
...
6. Invokes `Skill(skill: "appbuilder")` — the router's resume logic takes over and
   drives Stage B.
...
### Step 13 — Scaffold (`appbuilder-13-scaffold`)
...
cite real symbols, not CSS. Orchestrator + 1 `ab-implementer`.
...
### Step 14 — Implement (`appbuilder-14-implement`)
...
5. **Epic completion** — when an epic's last task finishes: spawn `ab-code-critic` on
   the epic's REAL git diff; findings go to `ab-patcher` (tool-locked Read+Edit);
...
2. Spawn `ab-implementer` — reads the idea, the PRD section, the feature file(s) the
...
3. Spawn `ab-test-engineer` — writes/extends tests for the task, runs them, fixes
...
### Step 15 — Adversarial review (`appbuilder-15-adversarial-review`)

Whole-app adversarial pass. Spawns 3 critics in parallel, ONE message:

- `ab-code-critic` — quality, security, idioms vs stack-guide + generated skills.
- `ab-spec-critic` — every must/should feature actually present and wired end-to-end.
- `ab-ux-critic` — SCREENSHOT COMPARISON: captures the implemented app's screens via
...
Findings JSONs are merged and ranked; `ab-patcher` applies surgical fixes, and the
...
### Step 16 — Ship gate (`appbuilder-16-ship-gate`)

Runs the [ship gate checklist](#gate-2--ship-gate-step-16) via 1 `ab-gate-verifier` —
...
1. **The verbatim idea, block-quoted** — plus the path to `idea.md` so the subagent can
   ...
   interesting.
2. **Pipeline position** — "You are step N (<name>) of the appbuilder pipeline. Step
...
prompt: |
  ...
  PIPELINE POSITION: You are step <N> (<step name>) of the appbuilder
...
Every check is a disk-verifiable fact, executed mechanically by `ab-gate-verifier` and
...
(summary + gallery path + `/appbuilder-choose a|b|c`). This is the ONE permitted stop.

## Gate 2 — ship gate (step 16)

Same mechanics: `ab-gate-verifier`, per-check evidence, artifacts-only fixes, max 3
...
## LINEAGE — what appbuilder copied from hyperresearch

Appbuilder is not "inspired by" hyperresearch in the loose sense; each mechanism below
is a deliberate port of a mechanism that hyperresearch proved under fire.

| Appbuilder mechanism | Hyperresearch ancestor | What carried over |
|---|---|---|
| `appbuilder` router + `appbuilder-N-*` step skills, each loaded fresh via `Skill()` | `hyperresearch` entry skill + 16 step skills (the V8 fix) | One giant skill gets compacted away mid-run and steps silently vanish; per-step skills load at the moment of need and survive context rot. Steps bounce control back through the router; they never chain directly. |
...
| Step 3.5 research audit — `ab-research-critic` refutes the corpus, clusters syndicated copies | hyperresearch's corpus critic / source-independence audit | Research is attacked BEFORE it is consumed: derivative copies count as ONE source, and every headline claim survives a refutation attempt or gets downgraded — never silently deleted. |
...
| ONE human stop — the design gate | (deliberate divergence) hyperresearch has zero mid-run checkpoints | Taste is the one thing the pipeline shouldn't decide. Appbuilder stops exactly once, for exactly one question — a|b|c — and nowhere else. |
