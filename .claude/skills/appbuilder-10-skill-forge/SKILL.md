---
name: appbuilder-10-skill-forge
description: >
  Step 10 of the appbuilder pipeline — generates the five PROJECT-SPECIFIC
  ...
  by spawning 5 ab-skill-smith subagents in parallel, each grounded in
  research/stack-guide.md + research/product-spec.md, bound by step 9's
  research/skill-authoring-guide.md, and adapting harvested skills from
  ...
  design-token references wired later by step 13. Invoked by the appbuilder
  router via Skill(); not run directly by users.
---

# Step 10 — Skill forge (parallel, 5 smiths)

You are executing step 10 (skill-forge) of the appbuilder pipeline. Step 9 produced
`research/skill-authoring-guide.md` (authoring law for everything written here); step 11
...
**⚠ CRITICAL: `.claude/skills/` is SHARED with this pipeline's own `appbuilder-*`
skills. Each smith writes ONLY its assigned `app-*` directory. If you find yourself (or
a smith) about to touch any `appbuilder-*` file, STOP — that is the harness, not the
product.**
...
- `research/skill-authoring-guide.md` — step 9's guide; its "Rules for our generated
  skills" section is LAW for every file this step writes
...
- `research/stack/architecture.md`, `research/stack/structure.md`,
  `research/stack/testing.md`, `research/stack/tooling-ci.md` — per-topic stack docs
- `features/00-index.md` — feature index (step 4.5)

**Pre-flight gate:** if `research/skill-authoring-guide.md` is missing or lacks its
...
2. **Spawn all 5 smiths in ONE message** — five Task calls, all
   `subagent_type: ab-skill-smith`, true parallel execution. One smith per skill, zero
   scope overlap. Assignments:

   | skill_name | scope (paste verbatim into the spawn) | primary inputs beyond the common set | extra_directives |
   |---|---|---|---|
   | `app-code-style` | Language idioms, naming, formatting, imports, error handling, and the lint/formatter rules this project commits to. Good/bad example pairs for each rule. | `research/stack/tooling-ci.md` | none |
   | `app-architecture` | Layering, module boundaries, state management, dependency direction, and "where does new code go" placement rules for this project's structure. | `research/stack/architecture.md`, `research/stack/structure.md` | none |
   | `app-testing` | Test pyramid and framework choices, test file naming/placement, what every task MUST cover (happy path, edge cases, error states), how to run the suite, coverage expectations. | `research/stack/testing.md` | none |
   ...
   | `app-review-checklist` | The concrete checklist ab-code-critic and the step 15 critics apply: checkable bullets covering style, architecture, testing, and UI conformance, each phrased as a yes/no question with the evidence to look for. | `research/stack/tooling-ci.md`, `research/stack/testing.md` | Derive ONLY from the shared ground truth (stack docs, PRD, authoring guide) — your four sibling skills do not exist yet; the orchestrator reconciles overlap after all five land. |
   ...
   **Spawn template** (fill `<>` placeholders from the table above):
   ...
   prompt: |
     ...
     PIPELINE POSITION: You are step 10 (skill-forge) of the appbuilder
     pipeline. Step 9 produced research/skill-authoring-guide.md — its
     ...
     - research/skill-authoring-guide.md   (authoring law — read the Rules
       section twice, and the Adaptable skills shortlist)
     - research/stack-guide.md             (committed stack decisions — your rules must agree, never contradict)
     - research/product-spec.md            (the PRD — personas, MoSCoW features, screen inventory)
     - runs/<run_tag>/decisions/platform.md
     - <primary inputs from the assignment table>
     ...
     - SKILL.md (~100-250 lines, the always-loaded core). Frontmatter:
       ...
       at research/stack-guide.md and research/product-spec.md.
     ...
     Do NOT write outside output_dir. Do NOT touch any appbuilder-*
...
4. **Validate every skill** once all five return. Mechanical checklist per skill:
   ...
   - No `appbuilder-*` file was modified (check `git status` / mtimes if available)
...
- No `appbuilder-*` skill was created, modified, or deleted

Then update the manifest: `steps.10 = done`, mark the step-10 todo complete, and return
to the router, which invokes `Skill(skill: "appbuilder-11-epics")`.
