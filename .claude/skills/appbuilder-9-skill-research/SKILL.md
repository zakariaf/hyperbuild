---
name: appbuilder-9-skill-research
description: >
  Step 9 of the appbuilder pipeline — deep research on Claude Code skill
  authoring: SKILL.md format, frontmatter fields, progressive disclosure,
  reference-file splitting, and richness norms. Spawns 2 ab-stack-researcher
  ...
  five ab-skill-smith spawns treat as law. Invoked by the appbuilder router
  via Skill(); not run directly by users.
---

# Step 9 — Skill research (parallel, 2 researchers)

You are executing step 9 (skill-research) of the appbuilder pipeline. Step 8 (mockups)
...
- `.claude/skills/appbuilder-*/SKILL.md` and `.claude/agents/ab-*.md` — the in-repo
...
2. **Spawn both researchers in ONE message** — true parallel execution. Two Task calls,
   both `subagent_type: ab-stack-researcher`. Assignments:
   ...
   **Spawn template** (fill the `<>` placeholders per the table; ZERO overlap between
   ...
   prompt: |
     ...
     PIPELINE POSITION: You are step 9 (skill-research) of the appbuilder
     ...
     - <exemplar-mining only: every .claude/skills/appbuilder-*/SKILL.md
       and .claude/agents/ab-*.md file in this repo — they ARE your corpus>
