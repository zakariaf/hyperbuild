---
name: appbuilder-2-market-recon
description: >
  Step 2 of the appbuilder pipeline — competitor recon. Spawns ONE
  ab-competitor-scout to discover and rank the competitor set, then a
  parallel wave of ab-competitor-analyst subagents (one per curated
...
### 2.1 — Spawn ONE ab-competitor-scout
...
```
subagent_type: ab-competitor-scout
prompt: |
  ...
  of the appbuilder pipeline. Step 1 resolved the target platform. After
  you return, the orchestrator curates your ranked list down to
  <6–8 standard | 12–15 premier> competitors and spawns one
  ab-competitor-analyst per competitor; their dossiers feed the PRD in
...
Spawn **one `ab-competitor-analyst` per non-reused competitor, all in ONE message** — true parallel execution. That is 6–8 Task calls for standard, 12–15 for premier. Zero overlap: each analyst gets exactly one competitor.
...
prompt: |
  ...
  the appbuilder pipeline. The ab-competitor-scout discovered the set; the
...
Return to the router (`appbuilder`). Step 3 (social mining) is your concurrent pair member — it mines what real users say about exactly these competitors, seeded by your curated list — and may already be running or done. Once BOTH members of the 2 ∥ 3 pair are done, the router invokes:
