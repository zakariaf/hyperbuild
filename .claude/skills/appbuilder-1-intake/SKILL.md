---
name: appbuilder-1-intake
description: >
  Step 1 of the appbuilder pipeline — persists the verbatim app idea as
...
You are executing step 1 (intake) of the appbuilder pipeline. Nothing has run before you — the user just typed `/appbuilder <idea>` and the router invoked this skill; step 2 (market recon) consumes the artifacts you write here, and EVERY subsequent step and subagent re-reads `idea.md` as gospel.
...
- The user's idea prompt — everything after the `/appbuilder ` command token, byte-for-byte. This is the ONLY input that exists yet.
...
7. **Write `runs/<run_tag>/decisions/platform.md`:**
   ...
   - `/appbuilder-choose <a|b|c> <platform>` can override; steps 5, 10, 11 then re-run.
   ```
...
8. **Initialize `runs/<run_tag>/manifest.json`** — the run's single source of truth for resume:
   ...
   Step keys are added as steps run: a step sets `steps.N = "running"` on entry and `"done"` on exit; a missing key means not started. The router resumes at the first step that isn't `"done"`. `stage` is `PLAN` now; `/appbuilder-choose` flips it to `BUILD`.
...
10. **Seed TodoWrite** — one todo per pipeline step (1 through 16, including 3.5 and 4.5) plus the checkpoint todo; 19 todos total, in order:
    ...
    14. Step 12 — design gate (STOP: wait for /appbuilder-choose)
    15. Checkpoint — /appbuilder-choose records the design choice (completed by the appbuilder-choose skill, never by a step)
...
Return to the router (`appbuilder`). It invokes:

```
Skill(skill: "appbuilder-2-market-recon")
```
