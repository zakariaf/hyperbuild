---
name: appbuilder-5-stack-research
description: >
  Step 5 of the appbuilder pipeline — spawns 4 parallel ab-stack-researcher
  ...
  it and the step-15 code critic enforces it. Invoked by the appbuilder
  router via Skill(); not run directly by users.
---

# Step 5 — Stack research (parallel, 4 researchers)

You are executing step 5 (stack-research) of the appbuilder pipeline. Steps 4 and 4.5 fixed WHAT to build (the PRD + feature specs); this step fixes HOW to build it — step 10 forges the project skills from the stack guide, step 13 scaffolds by it, step 14 implements by it, and step 15's `ab-code-critic` treats deviations from it as findings.
...
2. **Spawn 4 `ab-stack-researcher` subagents in ONE message** — all four Task calls together, true parallel execution. Each researcher gets ONLY its topic. Zero overlap: the `structure` researcher owns state management; the `architecture` researcher references that choice as an open input rather than making it. Spawn template (fill per topic from the table above):
   ...
   prompt: |
     ...
     <topic_id>) of the appbuilder pipeline. Step 1 chose the platform —
...
Return to the router (`appbuilder`). Invoke step 6:

```
Skill(skill: "appbuilder-6-design-research")
```
