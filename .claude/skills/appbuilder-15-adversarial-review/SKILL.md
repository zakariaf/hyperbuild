---
name: appbuilder-15-adversarial-review
description: >
  Step 15 of the appbuilder pipeline — whole-app adversarial review. Spawns
  3 critics in parallel (ab-code-critic, ab-spec-critic, ab-ux-critic),
  each emitting an independent findings JSON to runs/<run_tag>/gates/. The
  orchestrator ranks and dedupes findings, spawns ab-patcher (TOOL-LOCKED
...
You are executing step 15 (adversarial-review) of the appbuilder pipeline. Step 14 implemented every epic with per-epic reviews and green tests; step 16 (ship gate) runs next and its verdict is final — this step is the last chance to find and fix defects before the gate.
...
- `app/design/tokens.css` + `app/design/design-system.md` — the chosen design system (copied by /appbuilder-choose).
- `runs/<run_tag>/designs/<choice>/mockups/` — the chosen design's per-screen HTML mockups (step 8).
- `runs/<run_tag>/designs/<choice>/screenshots/` — headless-Chrome renders of those mockups (step 8) — ab-ux-critic's comparison baseline (absent only if the manifest logged `screenshots_skipped`).
...
2. **Resolve the chosen design.** Read `runs/<run_tag>/decisions/design-choice.md` and manifest `design_choice` — the letter fills `<choice>` in the ab-ux-critic template. If they disagree, `decisions/design-choice.md` wins; fix the manifest.

3. **Spawn all 3 critics in parallel.** In ONE message — three Task calls:
   - `ab-code-critic` → `runs/<run_tag>/gates/review-findings-code.json`
   - `ab-spec-critic` → `runs/<run_tag>/gates/review-findings-spec.json`
   - `ab-ux-critic` → `runs/<run_tag>/gates/review-findings-ux.json`
   ...
   **ab-code-critic spawn template:**
   ...
   prompt: |
     ...
     of the appbuilder pipeline. Step 14 implemented every epic; tests were
     green at each epic boundary. You run in parallel with ab-spec-critic
     and ab-ux-critic. After you return, the orchestrator ranks and dedupes
     all findings; ab-patcher (TOOL-LOCKED to Read + Edit) applies the
   ...
   **ab-spec-critic spawn template:**
   ```
   subagent_type: ab-spec-critic
   prompt: |
     APP IDEA (verbatim, gospel):
     > {{paste runs/<run_tag>/idea.md body}}
     ...
     of the appbuilder pipeline. Step 4 wrote the PRD, step 4.5 the feature
     specs, step 11 the epics, step 14 the implementation. You run in
     parallel with ab-code-critic and ab-ux-critic. After you return, the
     orchestrator ranks and dedupes; ab-patcher applies patchable findings;
   ...
   **ab-ux-critic spawn template:**
   ```
   subagent_type: ab-ux-critic
   prompt: |
     APP IDEA (verbatim, gospel):
     > {{paste runs/<run_tag>/idea.md body}}
     ...
     the appbuilder pipeline. Step 8 built HTML mockups of every screen in 3
     ...
     with ab-code-critic and ab-spec-critic. After you return, the
     orchestrator ranks and dedupes; ab-patcher applies patchable findings.
...
7. **Pre-create the patch log stub.** ab-patcher is tool-locked to `[Read, Edit]` — it cannot Write, and Edit only modifies existing files. You MUST stub the canonical schema first:
   ```bash
...
8. **Spawn ab-patcher.** Spawn ONCE, only if `patchable` is non-empty (if empty, note it in orchestrator-notes.md and go to item 10):
   ```
   ...
   prompt: |
     ...
     the appbuilder pipeline. Three critics reviewed the whole app; the
...
9. **Read the patch log; verify the app still stands.** When the patcher returns:
   ...
   - **Run the FULL test suite** with the exact test command recorded in `runs/<run_tag>/scaffold.md` `## Toolchain` (step 13 verified it — never re-derive build commands downstream). If red, spawn ab-test-engineer ONCE with the failing test list and the instruction: fix the code the patches broke, NEVER weaken or delete a test. Do not proceed with a red suite.
...
- `runs/<run_tag>/gates/review-merged.json` — orchestrator's ranked, deduped split:
  ...
  ```
- `runs/<run_tag>/gates/review-patch-log.json` — stubbed by orchestrator, populated by ab-patcher (canonical keys: total_findings, applied, skipped, conflicts, escalated — the same key set as step 14's per-epic patch logs).
...
- **Otherwise (no structural tasks, or second pass):** update manifest: `steps."15": "done"`, mark the step-15 todo complete, return to the router and invoke:
  ```
  Skill(skill: "appbuilder-16-ship-gate")
  ```
