...
hygiene: step 14 runs hb-code-critic on each EPIC'S DIFF, and step 14's crash recovery
...
### 13.4 Spawn ONE hb-implementer — tooling + theme
...
prompt: |
  ...
  PIPELINE POSITION: You are step 13 (scaffold) of the hyperbuild pipeline.
  ...
  what you wire here — every future hb-implementer inherits your lint config,
...
`run` and `screenshot_tests` are what step 15's hb-ux-critic derives its capture
...
the theme the way step 14 implementers should. Every step 14 hb-implementer reads this
...
Return to the router (`hyperbuild`). Invoke step 14:
