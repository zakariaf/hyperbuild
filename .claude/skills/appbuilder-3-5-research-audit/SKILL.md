---
name: appbuilder-3-5-research-audit
description: >
  Step 3.5 of the appbuilder pipeline — the adversarial research audit,
  running after steps 2 AND 3 (the 2 ∥ 3 concurrent pair) both
  complete. Spawns ONE ab-research-critic to attack
...
2. **Spawn ONE `ab-research-critic` to attack the surface.** Spawn template:

   ```
   subagent_type: ab-research-critic
   prompt: |
     ...
     appbuilder pipeline. Steps 2 (market recon) and 3 (social mining)
...
```
Skill(skill: "appbuilder-4-product-spec")
```
