---
name: hyperbuild-choose
description: >
  Human checkpoint of the hyperbuild pipeline — the user's ONE decision.
  Invoked directly by the user as /hyperbuild-choose <a|b|c> [platform] after
  ...
  and 11 for re-run), then invokes Skill(skill: "hyperbuild") so the router's
...
1. **Locate the run.** Glob `runs/*/manifest.json`. Prefer the manifest with
   ...
   records nothing:
   - No manifest at all → "No hyperbuild run found. Start one with
     `/hyperbuild <your app idea>`."
   - `stage: "PLAN"` but `steps["12"]` is not `"done"` → "The run
     `<run_tag>` hasn't reached the design gate yet. Resume it with
     `/hyperbuild` — I'll stop and ask for your choice when the three designs
     are ready."
...
2. **Validate the gate state on disk.** All of these must exist and be
   ...
   missing path, and tell the user to re-run `/hyperbuild` (the router will
   re-enter step 12):
...
4. **Write the decision record** to `runs/<run_tag>/decisions/design-choice.md`.
   ...
   The user chose design **b** ("Swiss Utility") via `/hyperbuild-choose b`.
...
5. **Apply the platform override (only if arg 2 differs from
   ...
   - Append a `## Overridden at checkpoint` section to
     ...
     "user override via /hyperbuild-choose", timestamp.
...
```
Skill(skill: "hyperbuild")
```

**NEVER invoke `hyperbuild-13-scaffold` (or any step skill) directly from
