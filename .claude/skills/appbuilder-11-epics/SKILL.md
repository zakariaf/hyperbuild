---
name: appbuilder-11-epics
description: >
  Step 11 of the appbuilder pipeline — turns the PRD and feature specs into
  ...
  contract step 14 keys off. Spawns 1 ab-epic-planner (orchestrator approves
  the breakdown), then parallel ab-task-author subagents (one per epic),
  then an ab-spec-critic coverage audit — every must/should PRD feature maps
  to ≥1 task. Invoked by the appbuilder router via Skill(); not run directly
  by users.
---

# Step 11 — Epics & tasks (the full backlog)

You are executing step 11 (epics) of the appbuilder pipeline. Step 10 forged the
...
PRD feature traces to ≥1 task, each executable by a fresh ab-implementer with zero
...
### Step 11.1 — Spawn ONE `ab-epic-planner` (the breakdown draft)

**Spawn template:**
```
subagent_type: ab-epic-planner
prompt: |
  ...
  PIPELINE POSITION: You are step 11 (epic planner) of the appbuilder
  ...
  approves it, then parallel ab-task-author subagents (one per epic) write
...
### Step 11.3 — Spawn `ab-task-author` subagents, ONE PER EPIC, in parallel
...
prompt: |
  ...
  PIPELINE POSITION: You are step 11 (task author) of the appbuilder
  ...
  matrix and an ab-spec-critic audits it; in Stage B, step 14 implementers
...
### Step 11.5 — Spawn ONE `ab-spec-critic` (coverage audit)
...
prompt: |
  ...
  PIPELINE POSITION: You are the step 11 coverage auditor of the appbuilder
...
`ab-task-author` with `assigned_features` narrowed to the gap ids. Matrix drift →
re-derive `00-overview.md` from disk. Then re-spawn `ab-spec-critic` fresh. **Max 3
...
Return to the router (`appbuilder`). Invoke step 12:

```
Skill(skill: "appbuilder-12-design-gate")
```
